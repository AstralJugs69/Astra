  ...
  <main>
    ...
    <section aria-labelledby="human-disposition">
      ...
      <form method="post" action="/incidents/{{ incident_id }}/professional-dispositions">
        ...
        <label>Decision <select name="decision" required><option value="" selected disabled>Choose a decision</option>{% for decision in decisions %}<option value="{{ decision }}">{{ decision }}</option>{% endfor %}</select></label>
...
<section class="card"><h2>Executive decision summary</h2><p><strong>Recorded outcome or current gate:</strong> {{ display.cockpit.status }}</p><p><strong>Current human role:</strong> {{ display.cockpit.role }}</p><p><strong>Required action:</strong> {{ display.cockpit.action }}</p><p>{{ display.cockpit.message }}</p><p>{{ next_safe_action }}</p>{% if review_state.blocking_reason %}<p class="callout"><strong>Blocking reason:</strong> {{ review_state.blocking_reason }}</p>{% endif %}</section>
...
def _next_safe_action(review_state: Mapping[str, object]) -> str:
    ...
    if state == "HALT_REQUESTED":
        return (
            "The halt request is recorded; authorized containment remains external and human-owned."
        )
    if state == "CONTINUE_ACCEPTED":
        return "The coordinator accepted continuation; Astra recorded the decision and took no production action."
    if state == "DEFERRED":
        return "The coordinator deferred the decision; no containment or closure is claimed."
    if state == "REPORT_REJECTED":
        return "The report was rejected; correct its evidence before reassessment."
    if state == "RESOLVED_NO_REMEDIATION_BY_HUMAN":
        return "The human workflow is complete without remediation."
    if state == "RESOLVED_BY_HUMAN":
        return "The human workflow is complete; preserve final verification evidence."
