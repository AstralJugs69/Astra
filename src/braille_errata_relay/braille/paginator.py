...
class BraillePage:
    ...
    def source_block_ids(self) -> tuple[str, ...]:
        return tuple(
            dict.fromkeys(block_id for line in self.lines for block_id in line.source_block_ids)
        )


def paginate(
    lines: tuple[FormattedLine, ...], profile: TranslationProfile
) -> tuple[BraillePage, ...]:
    if not lines:
        return (BraillePage(1, ()),)
