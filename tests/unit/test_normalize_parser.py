...
def test_normalization_is_line_ending_and_trailing_space_invariant() -> None:
    first = normalize_source_bytes(
        b"# Section\r\n\r\nA sentence.  \r\n", document_id="fixture"
    )
    second = normalize_source_bytes(
        "# Section\n\nA sentence.\n".encode(), document_id="fixture"
    )
    assert first == second
    assert first.blocks[1].block_id == "section/p-001"
...
def test_changed_block_has_stable_section_id_and_bounded_evidence() -> None:
    ...
    assert source_diff.changed_block_ids == ("biology/p-001",)
    assert evidence_span_ids(source_diff) == ("old:biology/p-001", "new:biology/p-001")
